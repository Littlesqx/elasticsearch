...
在 5.x 被创建的多映射类型在 6.x 会继续像以前一样工作。而在 7.0.0 ，映射类型将会被完全移除。
...
<<docs-reindex,Reindex API>> 可用于将多类型索引转换为单类型索引。
下面的例子可用于 Elasticsearch 5.6 或 Elasticsearch 6.x 。
在 6.x 中，不需要指定 `index.mapping.single_type` ，因为那是默认值。

[float]
==== 索引每个文档类型

第一个例子是将我们的 `twitter` 索引分成一个 `tweets` 索引和一个 `users` 索引：
...
下面一个例子是添加一个自定义 `type` 字段，并且将它设置为原始 `_type` 的值。它还需要将类型添加到 `_id` ，
以防止不同类型的某些文档存在冲突的 IDs 。
